...
const INSTRUMENTS: Instrument[] = [
  // Pesos
  { id: 'pf_tradicional', name: 'Plazo Fijo Tradicional', shortName: 'PF Tradl.', category: 'Pesos', annualReturn: 37, color: '#1D9BF0', description: 'Tasa fija garantida por el banco. Liquidez al vencimiento.', risk: 'bajo' },
  ...
  // Dólares
  { id: 'dolar_mep', name: 'Dólar MEP (hold)', shortName: 'USD MEP', category: 'Dólares', annualReturn: 15, color: '#1D9BF0', description: 'Devaluación esperada del tipo de cambio oficial + carry.', risk: 'medio' },
  { id: 'obligacion', name: 'Obligación Negociable', shortName: 'ON Hard $', category: 'Dólares', annualReturn: 8, color: '#1A8CD8', description: 'Bono corporativo en dólares. YPF, Pampa, etc.', risk: 'medio' },
  // Acciones
  ...
  { id: 'cedear_spy', name: 'SPY CEDEAR (S&P500)', shortName: 'SPY', category: 'CEDEARs', annualReturn: 25, color: '#1D9BF0', description: 'ETF del S&P 500 en pesos. Sube con el CCL.', risk: 'medio' },
...
export default function Simulador() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-[#E7E9EA] font-semibold text-base">Simulador de Inversiones</h1>
        <p className="text-[#71767B] text-xs mt-0.5">Compará rendimientos proyectados · Basado en tasas históricas promedio</p>
      </div>

      {/* Controls */}
      <div className="bg-[#16181C] border border-[#2F3336] rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          <Calculator size={15} className="text-[#1D9BF0]" />
          <span className="text-[#E7E9EA] font-semibold text-sm">Parámetros</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label className="text-[#71767B] text-xs block mb-1.5">Capital inicial (ARS)</label>
            <input
              type="number"
              value={principal}
              onChange={e => setPrincipal(Number(e.target.value))}
              className="w-full bg-[#000000] border border-[#2F3336] rounded-lg px-3 py-2 text-[#E7E9EA] font-mono text-sm outline-none focus:border-[#3E4144] tabular-nums"
            />
                ...
                <button
                  ...
                  className={`text-[10px] px-2 py-0.5 rounded transition-colors ${
                    principal === v
                      ...
                      : 'bg-[#2F3336] text-[#71767B] hover:text-[#E7E9EA]'
                  }`}
                >
                  {fmtARS(v)}
                </button>
          ...
          <div>
            <label className="text-[#71767B] text-xs block mb-1.5">Horizonte: {months} meses</label>
            <input
              type="range"
              min={1}
              max={60}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Instrument selector */}
        <div className="bg-[#16181C] border border-[#2F3336] rounded-xl p-5">
          <div className="text-[#E7E9EA] font-semibold text-sm mb-1">Instrumentos</div>
          <div className="text-[#71767B] text-xs mb-4">Seleccioná hasta 4 para comparar</div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-semibold" style={{ color: isOn ? inst.color : '#8B98A5' }}>
                            {inst.name}
                          </span>
                          <span
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {finalValues.length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {finalValues.map(({ inst, final, gain, pct }) => (
                <div
                  key={inst.id}
                  className="bg-[#16181C] border rounded-xl p-4"
                  style={{ borderColor: `${inst.color}30` }}
                >
                  <div className="text-xs font-semibold mb-2" style={{ color: inst.color }}>
                    {inst.shortName}
                  </div>
                  <div className="font-mono font-bold text-[#E7E9EA] text-sm tabular-nums">{fmtARS(final)}</div>
                  <div className="font-mono text-[#1D9BF0] text-xs tabular-nums">+{fmtARS(gain)}</div>
                  <div className="flex items-center gap-1 mt-1">
                    <TrendingUp size={11} className="text-[#1D9BF0]" />
                    <span className="text-[#1D9BF0] text-[10px] font-mono">+{pct.toFixed(1)}%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-[#16181C] border border-[#2F3336] rounded-xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[#E7E9EA] font-semibold text-sm">Proyección a {months} meses</span>
            </div>
            {selectedInstruments.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={chartData}>
                  ...
                  />
                  <Legend wrapperStyle={{ fontSize: 11, color: '#71767B' }} />
                  {selectedInstruments.map(inst => (
                    <Area
                      key={inst.id}
                      type="monotone"
          ...
          {/* Disclaimer */}
          <div className="flex gap-2 text-[#475569] text-xs bg-[#000000] border border-[#2F3336] rounded-xl p-3">
