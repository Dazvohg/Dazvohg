...
const NAV_ITEMS = [
  { to: '/app/mango/game',            icon: LayoutDashboard, label: 'Inicio',    end: true },
  { to: '/app/mango/game/market',     icon: ShoppingCart,    label: 'Mercado',   end: false },
  { to: '/app/mango/game/mapa',       icon: Map,             label: 'Mapa',      end: false },
  { to: '/app/mango/game/empresa',    icon: Building2,       label: 'Empresa',   end: false },
  { to: '/app/mango/game/objectives', icon: Target,          label: 'Objetivos', end: false },
  { to: '/app/mango/game/academia',   icon: GraduationCap,   label: 'Academia',  end: false },
]
      ...
      <nav className="fixed bottom-0 inset-x-0 z-50 bg-gray-950/95 backdrop-blur border-t border-gray-800 safe-area-pb">
        <div className="flex items-center justify-around py-1 px-1 max-w-lg mx-auto overflow-x-auto scrollbar-hide">
          {NAV_ITEMS.map(({ to, icon: Icon, label, end }) => {
            const isObjectives = to === '/app/mango/game/objectives'
            return (
              <NavLink
                key={to}
                to={to}
